...
describe Pantograph::Actions do
  describe "#sh" do
    before do
      allow(PantographCore::Helper).to receive(:sh_enabled?).and_return(true)
    end

    context "external commands are failed" do
      context "with error_callback" do
        it "doesn't raise shell_error" do
          allow(PantographCore::UI).to receive(:error)
          called = false
          expect_command("exit 1", exitstatus: 1)
          Pantograph::Actions.sh("exit 1", error_callback: ->(_) { called = true })

          expect(called).to be(true)
          expect(PantographCore::UI).to have_received(:error).with("Exit status of command 'exit 1' was 1 instead of 0.\n")
        end
      end

      context "without error_callback" do
        it "raise shell_error" do
          allow(PantographCore::UI).to receive(:shell_error!)
          expect_command("exit 1", exitstatus: 1)
          Pantograph::Actions.sh("exit 1")
    ...
    context "passing command arguments to the system" do
      it "passes a string as a string" do
        ...
        Pantograph::Actions.sh("git commit")
      end
      ...
      it "passes an environment Hash" do
        ...
        Pantograph::Actions.sh({ "PATH" => "/usr/local/bin" }, "git", "commit")
      end

      it "allows override of argv[0]" do
        expect_command(["/usr/local/bin/git", "git"], "commit", "-m", "A message")
        Pantograph::Actions.sh(["/usr/local/bin/git", "git"], "commit", "-m", "A message")
      end

      it "allows a single array to be passed to support older Pantograph syntax" do
        expect_command("ls -la /tmp")
        Pantograph::Actions.sh(["ls -la", "/tmp"])
      end
    end
    ...
    context "with a postfix block" do
      it "yields the status, result and command" do
        ...
        Pantograph::Actions.sh("ls", "-la") do |status, result, command|
      ...
      it "yields any error result" do
        ...
        Pantograph::Actions.sh("ls", "-la") do |status, result|
      ...
      it "returns the return value of the block if present" do
        expect_command("ls", "-la")
        return_value = Pantograph::Actions.sh("ls", "-la") do |status, result|
          42
        end
...
def command_from_args(*args)
  Pantograph::Actions.shell_command_from_args(*args)
end
