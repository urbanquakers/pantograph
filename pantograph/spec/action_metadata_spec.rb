...
describe Pantograph::Action do
  Pantograph::ActionsList.all_actions do |action, name|
    describe name do
      it "`pantograph_class` and `action` are matching" do
        # `to_s.gsub(/::.*/, '')` to convert
        #   "Pantograph::Actions::AdbDevicesAction"
        # to
        #   "AdbDevicesAction"
        #
        expect(name.pantograph_class + "Action").to eq(action.to_s.gsub(/^.*::/, ''))
      end
      ...
      it "file name follows our convention and matches the class name" do
        ...
        action_path = File.join(Dir.pwd, "pantograph/lib/pantograph/actions/#{name}.rb")
      ...
      it "contains a valid category" do
        ...
        expect(Pantograph::Action::AVAILABLE_CATEGORIES).to include(action.category), "Unknown action category '#{action.category}', must be one of #{Pantograph::Action::AVAILABLE_CATEGORIES.join(', ')}"
      end

      it "is a subclass of Action" do
        expect(action.ancestors.include?(Pantograph::Action)).to be_truthy, "Please add `Action` as a superclass for action '#{name}'"
      end
