  ...
  describe Pantograph::PantFile do
    describe "Nexus Upload integration" do
      it "sets verbosity correctly" do
        expect(Pantograph::Actions::NexusUploadAction.verbose(verbose: true)).to eq("--verbose")
        expect(Pantograph::Actions::NexusUploadAction.verbose(verbose: false)).to eq("--silent")
      end

      it "sets upload url correctly for Nexus 2" do
        expect(Pantograph::Actions::NexusUploadAction.upload_url(endpoint: "http://localhost:8081",
      ...
      it "sets upload url correctly for Nexus 3 with all required parameters" do
        ...
        expect(Pantograph::Actions::NexusUploadAction.upload_url(
                 ...
                 repo_project_version: '1.12',
                   file: file_path.to_s
        )).to eq("http://localhost:8081/nexus/repository/artefacts/com/pantograph/myproject/1.12/myproject-1.12.ipa")
      end
      ...
      it "sets upload url correctly for Nexus 3 with repo classifier" do
        ...
        expect(Pantograph::Actions::NexusUploadAction.upload_url(
                 ...
                 repo_project_version: '1.12',
                   file: file_path.to_s,
                   repo_classifier: 'ipa'
        )).to eq("http://localhost:8081/nexus/repository/artefacts/com/pantograph/myproject/1.12/myproject-1.12-ipa.ipa")
      end
      ...
      it "sets upload options correctly for Nexus 2 with all required parameters" do
        ...
        result = Pantograph::Actions::NexusUploadAction.upload_options(
          nexus_version: 2,
        repo_id: 'artefacts',
        repo_group_id: 'com.pantograph',
        repo_project_name: 'myproject',
        ...
        expect(result).to include('-F g=com.pantograph')
      ...
      it "sets ssl option correctly" do
        expect(Pantograph::Actions::NexusUploadAction.ssl_options(ssl_verify: false)).to eq(["--insecure"])
        expect(Pantograph::Actions::NexusUploadAction.ssl_options(ssl_verify: true)).to eq([])
      end

      it "sets proxy options correctly" do
        expect(Pantograph::Actions::NexusUploadAction.proxy_options(proxy_address: "",
          proxy_port: nil, proxy_username: nil, proxy_password: nil)).to eq([])
        expect(Pantograph::Actions::NexusUploadAction.proxy_options(proxy_address: nil,
          proxy_port: "", proxy_username: nil, proxy_password: nil)).to eq([])
        expect(Pantograph::Actions::NexusUploadAction.proxy_options(proxy_address: nil,
          proxy_port: nil, proxy_username: "", proxy_password: "")).to eq([])
        expect(Pantograph::Actions::NexusUploadAction.proxy_options(proxy_address: "http://1",
      ...
      it "runs the correct command for Nexus 3" do
        ...
        expect(result).to include('-u admin:admin123')
        expect(result).to include('--verbose')
        expect(result).to include('http://localhost:8081/nexus/repository/artefacts/com/pantograph/myproject/1.12/myproject-1.12.ipa')
        expect(result).to include('-x http://server:30')
        expect(result).to include('--proxy-user admin:admin')
        expect(result).to include('--insecure')
      end
