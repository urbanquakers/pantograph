    ...
    describe "git_add" do
      ...
      context "as string" do
        ...
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add #{path}", anything).and_return("")
      ...
      context "as array" do
        ...
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add #{path[0]} #{path[1]}", anything).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
            git_add(path: #{path})
          end").runner.execute(:test)
        end
      ...
      context "as string with spaces in name" do
        ...
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add #{path.shellescape}", anything).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
            git_add(path: '#{path}')
          end").runner.execute(:test)
        end
      ...
      context "as array with spaces in name and directory" do
        ...
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add #{path[0].shellescape} #{path[1].shellescape}", anything).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
            git_add(path: #{path})
          end").runner.execute(:test)
        end
      ...
      context "as string with wildcards" do
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add *.txt", anything).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
      ...
      context "as array with wildcards" do
        ...
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add *.h *.m", anything).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
      ...
      context "without parameters" do
        it "executes the correct git command" do
          allow(Pantograph::Actions).to receive(:sh).with("git add .", anything).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
            git_add
          end").runner.execute(:test)
        end
      ...
      it "logs the command if verbose" do
        PantographSpec::Env.with_verbose(true) do
          allow(Pantograph::Actions).to receive(:sh).with(anything, { log: true }).and_return("")
          result = Pantograph::PantFile.new.parse("lane :test do
      ...
      it "cannot have both path and pathspec parameters" do
        ...
        end.to raise_error(PantographCore::Interface::PantographError)
      end
    end
  end
end
