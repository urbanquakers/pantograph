  ...
  describe Pantograph::PantFile do
    describe "changelog_from_git_commits" do
      ...
      it "Does not accept an invalid value for :merge_commit_filtering" do
        values = Pantograph::Actions::GIT_MERGE_COMMIT_FILTERING_OPTIONS.map { |o| "'#{o}'" }.join(', ')
      ...
      it "Runs between option from command line" do
        expect(system("pantograph run changelog_from_git_commits between:123456,HEAD")).to be
      end
