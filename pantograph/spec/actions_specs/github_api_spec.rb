describe Pantograph do
  describe Pantograph::PantFile do
    describe "github_api" do
      let(:response_body) { File.read("./pantograph/spec/fixtures/requests/github_create_file_response.json") }
      let(:user_agent) { 'pantograph-github_api' }
      let(:headers) do
        {
      ...
      context 'successful' do
        before do
          stub_request(:put, "https://api.github.com/repos/pantograph/pantograph/contents/TEST_FILE.md").
            ...
            result = Pantograph::PantFile.new.parse("
              lane :test do
                github_api(
                  api_token: '123456789',
                  http_method: 'PUT',
                  path: 'repos/pantograph/pantograph/contents/TEST_FILE.md',
                  body: {
                    path: 'TEST_FILE.md',
        ...
        it 'allows calling as a block for success from other actions' do
          expect do
            Pantograph::PantFile.new.parse(%{
              lane :test do
                Pantograph::Actions::GithubApiAction.run(
            ...
            raise_error(PantographCore::Interface::PantographError) do |error|
        ...
        context 'optional params' do
          let(:response_body) { File.read("./pantograph/spec/fixtures/requests/github_upload_release_asset_response.json") }
          let(:headers) do
            {
              'Authorization' => 'Basic MTIzNDU2Nzg5',
          ...
          before do
            stub_request(:post, "https://uploads.github.com/repos/pantograph/pantograph/releases/1/assets?name=TEST_FILE.md").
              ...
              result = Pantograph::PantFile.new.parse(%{
                lane :test do
                  github_api(
                    api_token: '123456789',
                    http_method: 'POST',
                    url: 'https://uploads.github.com/repos/pantograph/pantograph/releases/1/assets?name=TEST_FILE.md',
          ...
          context "path is set, server_url isn't set" do
            it "uses default server_url" do
              expect do
                ...
                ").runner.execute(:test)
                expect(result[:status]).to eq(200)
                expect(result[:html_url]).to eq("https://api.github.com/repos/pantograph/pantograph/contents/TEST_FILE.md")
              end
            end
          end
          ...
          context "path and server_url are set" do
            it "correctly submits using the path and server_url instead of the url" do
              expect do
                result = Pantograph::PantFile.new.parse("
                    lane :test do
                      github_api(
                        api_token: '123456789',
                        http_method: 'PUT',
                        url: 'https://api.github.com/repos/pantograph/pantograph/contents/NONEXISTENT_TEST_FILE.md',
                        path: 'repos/pantograph/pantograph/contents/TEST_FILE.md',
                        server_url: 'https://api.github.com'
                      )
                    end
                ...
                expect(result[:status]).to eq(200)
                expect(result[:html_url]).to eq("https://api.github.com/repos/pantograph/pantograph/contents/TEST_FILE.md")
                expect(result[:html_url]).to_not(eq("https://api.github.com/repos/pantograph/pantograph/contents/NONEXISTENT_TEST_FILE.md"))
              end
            end
          end
                ...
                result = Pantograph::PantFile.new.parse("
                    lane :test do
                      github_api(
                        api_token: '123456789',
                        http_method: 'PUT',
                        url: 'https://api.github.com/repos/pantograph/pantograph/contents/TEST_FILE.md'
                      )
                    end
                  ").runner.execute(:test)
