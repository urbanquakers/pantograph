    ...
    describe "Clipboard Integration" do
      if PantographCore::Helper.mac?
        it "properly stores the value in the clipboard" do
          ...
          value = Pantograph::PantFile.new.parse("lane :test do
            clipboard(value: '#{str}')
      ...
      it "raises an error if the value is passed without a hash" do
        ...
        end.to raise_error("You have to call the integration like `clipboard(key: \"value\")`. Run `pantograph action clipboard` for all available keys. Please check out the current documentation on GitHub.")
      end
    end
  end
end
