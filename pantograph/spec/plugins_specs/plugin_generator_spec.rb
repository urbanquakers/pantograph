...
describe Pantograph::PluginGenerator do
  describe '#generate' do
    let(:plugin_info) { Pantograph::PluginInfo.new('tester_thing', 'Fabricio Devtoolio', 'fabric.devtools@gmail.com', 'summary', 'details') }
    ...
    before(:each) do
      ...
      unless initialized
        test_ui = Pantograph::PluginGeneratorUI.new
        ...
        generator = Pantograph::PluginGenerator.new(ui: test_ui, dest_root: tmp_dir)
    ...
    it "creates a plugin.rb file for the plugin" do
      plugin_rb_file = File.join(tmp_dir, gem_name, 'lib', 'pantograph', 'plugin', "#{plugin_name}.rb")
      ...
      Dir.chdir(gem_name) do
        ...
        # the behavior!
        action_class = Object.const_get("Pantograph::#{plugin_name.pantograph_class}")
    ...
    it "creates a Action class" do
      ...
      # behavior!
      action_class = Object.const_get("Pantograph::Actions::#{plugin_name.pantograph_class}Action")
    ...
    it "creates a complete, valid gemspec" do
      ...
      Dir.chdir(gem_name) do
        ...
        expect(gemspec.development_dependencies).to contain_exactly(
          ...
          Gem::Dependency.new("rubocop", Gem::Requirement.new([Pantograph::RUBOCOP_REQUIREMENT]), :development),
          ...
          Gem::Dependency.new("pantograph", Gem::Requirement.new([">= #{Pantograph::VERSION}"]), :development)
        )
      end
    end
    ...
    it "creates a valid helper class" do
      ...
      # syntax is valid, and lets us interrogate the class for its
      # behavior!
      helper_class = Object.const_get("Pantograph::Helper::#{plugin_name.pantograph_class}Helper")
    ...
    describe "All tests and style validation of the new plugin are passing" do
      before (:all) do
        ...
        plugin_sh('bundle install', 'pantograph-plugin-tester_thing')
      end
