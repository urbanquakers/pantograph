describe Pantograph::PluginInfoCollector do
  let(:test_ui) do
    ui = Pantograph::PluginGeneratorUI.new
  ...
  before do
    ["my plugin", "test_name", "my_", "pantograph-whatever", "whatever"].each do |current|
      stub_plugin_exists_on_rubygems(current, false)
    end
  end

  let(:collector) { Pantograph::PluginInfoCollector.new(test_ui) }
  ...
  describe "#collect_plugin_name" do
    ...
    describe "with invalid initial input" do
      it "offers a correction and uses it if accepted" do
        ...
        expect(collector.collect_plugin_name('pantograph-whatever')).to eq('whatever')
      end
  ...
  describe '#plugin_name_valid?' do
    ...
    it "handles plugin name containing 'pantograph'" do
      expect(collector.plugin_name_valid?('test_pantograph_name')).to be_falsey
    end
    ...
    it "handles plugin name starting with 'Pantograph '" do
      expect(collector.plugin_name_valid?('Pantograph test_name')).to be_falsey
    end

    it "handles plugin name starting with '#{Pantograph::PluginManager::PANTOGRAPH_PLUGIN_PREFIX}'" do
      expect(collector.plugin_name_valid?("#{Pantograph::PluginManager::PANTOGRAPH_PLUGIN_PREFIX}test_name")).to be_falsey
    end
  ...
  describe '#fix_plugin_name' do
    ...
    it "handles plugin name containing 'pantograph'" do
      expect(collector.fix_plugin_name('test_pantograph_name')).to eq('test_name')
    end
    ...
    it "handles plugin name starting with 'Pantograph '" do
      expect(collector.fix_plugin_name('Pantograph test_name')).to eq('test_name')
    end

    it "handles plugin name starting with '#{Pantograph::PluginManager::PANTOGRAPH_PLUGIN_PREFIX}'" do
      expect(collector.fix_plugin_name("#{Pantograph::PluginManager::PANTOGRAPH_PLUGIN_PREFIX}test_name")).to eq('test_name')
    end
  ...
  describe "author detection" do
    it "has an email from git config" do
      expect(PantographCore::Helper).to receive(:backticks).with('git config --get user.name', print: PantographCore::Globals.verbose?).and_return('Fabricio Devtoolio')
      expect(collector.detect_author).to eq('Fabricio Devtoolio')
    end

    it "does not have an email from git config" do
      expect(PantographCore::Helper).to receive(:backticks).with('git config --get user.name', print: PantographCore::Globals.verbose?).and_return('')
  ...
  describe "email detection" do
    it "has an email from git config" do
      expect(PantographCore::Helper).to receive(:backticks).with('git config --get user.email', print: PantographCore::Globals.verbose?).and_return('fabric.devtools@gmail.com')
      expect(collector.detect_email).to eq('fabric.devtools@gmail.com')
    end

    it "does not have email in git config" do
      expect(PantographCore::Helper).to receive(:backticks).with('git config --get user.email', print: PantographCore::Globals.verbose?).and_return('')
  ...
  describe '#collect_info' do
    it "returns a PluginInfo summarizing the user input" do
      ...
      info = Pantograph::PluginInfo.new('test_name', 'Fabricio Devtoolio', 'fabric.devtools@gmail.com', 'summary', 'details')
