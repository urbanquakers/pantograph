describe PantographCore do
  describe PantographCore::ConfigurationFile do
    describe "Properly loads and handles various configuration files" do
      let(:options) do
        [
          PantographCore::ConfigItem.new(key: :devices,
                                       description: "desc",
                                       is_string: false,
                                       optional: true),
          PantographCore::ConfigItem.new(key: :a_boolean,
                                       description: "desc",
                                       is_string: false,
                                       optional: true),
          PantographCore::ConfigItem.new(key: :another_boolean,
                                       description: "desc",
                                       is_string: false,
                                       optional: true),
          ...
          PantographCore::ConfigItem.new(key: :apple_id,
                                       description: "yo",
                                       default_value: "123")
        ]
      end
      ...
      it "overwrites existing values" do
        # not overwrite
        config = PantographCore::Configuration.create(options, { app_identifier: "detlef.app.super" })
      ...
      it "prints a warning if no value is provided" do
        important_message = "In the config file './pantograph_core/spec/fixtures/ConfigFileEmptyValue' you have the line apple_id, but didn't provide any value. Make sure to append a value right after the option name. Make sure to check the docs for more information"
        expect(PantographCore::UI).to receive(:important).with(important_message)
        expect(PantographCore::UI).to receive(:important).with("No values defined in './pantograph_core/spec/fixtures/ConfigFileEmptyValue'")
      ...
      it "supports modifying of frozen strings too" do
        ...
        PantographSpec::Env.with_env_values('SOMETHING_RANDOM_APP_IDENTIFIER' => app_identifier) do
          config = PantographCore::Configuration.create(options, {})
      ...
      describe "Handling invalid broken configuration files" do
        it "automatically corrects invalid quotations" do
          config = PantographCore::Configuration.create(options, {})
          config.load_configuration_file('./pantograph_core/spec/fixtures/ConfigInvalidQuotation')
        ...
        it "properly shows an error message when there is a syntax error in the Pantfile" do
          config = PantographCore::Configuration.create(options, {})
          expect do
            config.load_configuration_file('./pantograph_core/spec/fixtures/ConfigSytnaxError')
      ...
      describe "Prints out a table of summary" do
        it "shows a warning when no values were found" do
          expect(PantographCore::UI).to receive(:important).with("No values defined in './pantograph_core/spec/fixtures/ConfigFileEmpty'")
        ...
        it "prints out a table of all the set values" do
          expect(Terminal::Table).to receive(:new).with({
            ...
            title: "Detected Values from './pantograph_core/spec/fixtures/ConfigFileValid'"
          })
      ...
      describe "for_lane and for_platform support" do
        ...
        it "reads global keys when platform and lane dont match" do
          PantographSpec::Env.with_env_values('PANTOGRAPH_PLATFORM_NAME' => 'osx', 'PANTOGRAPH_LANE_NAME' => 'debug') do
            config = PantographCore::Configuration.create(options, {})
            config.load_configuration_file('ConfigFileForLane')
        ...
        it "reads lane setting when platform doesn't match or no for_platform" do
          PantographSpec::Env.with_env_values('PANTOGRAPH_PLATFORM_NAME' => 'osx', 'PANTOGRAPH_LANE_NAME' => 'enterprise') do
            config = PantographCore::Configuration.create(options, {})
            config.load_configuration_file('ConfigFileForLane')
