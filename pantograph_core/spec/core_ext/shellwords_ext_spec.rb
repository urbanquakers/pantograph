...
def confirm_shell_unescapes_string_correctly(string, escaped)
  ...
  if PantographCore::CommandExecutor.which('grep')
  ...
  elsif PantographCore::CommandExecutor.which('find')
    compare_string = simulate_normal_shell_unwrapping(compare_string)
...
describe "monkey patch of String.shellescape (via CrossplatformShellwords)" do
  describe "on Windows" do
    before(:each) do
      allow(PantographCore::Helper).to receive(:windows?).and_return(true)
    end
  ...
  describe "on other OSs (macOS, Linux)" do
    before(:each) do
      allow(PantographCore::Helper).to receive(:windows?).and_return(false)
    end
