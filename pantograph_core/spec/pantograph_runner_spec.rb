describe Commander::Runner do
  ...
  describe '#handle_unknown_error' do
    ...
    it 'should abort and show custom info for errors that have the Apple error info provider method with PantographCore::Globals.verbose?=false' do
      runner = Commander::Runner.new
      expect(runner).to receive(:abort).with("\n[!] Title\n\tLine 1\n\tLine 2".red)

      PantographSpec::Env.with_verbose(false) do
        runner.handle_unknown_error!(CustomError.new)
      end
    end

    it 'should reraise and show custom info for errors that have the Apple error info provider method with PantographCore::Globals.verbose?=true' do
