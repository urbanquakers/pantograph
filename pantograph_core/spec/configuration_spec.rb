  ...
  describe PantographCore::Configuration do
    describe "Create a new Configuration Manager" do
      ...
      it "raises an error if the option of a given value is not available" do
        expect do
          PantographCore::Configuration.create([], { cert_name: "value" })
      ...
      it "raises an error if a description ends with a ." do
        expect do
          PantographCore::Configuration.create([PantographCore::ConfigItem.new(
            key: :cert_name,
       env_name: "asdf",
    description: "Set the profile name."
      ...
      describe "config conflicts" do
        ...
        it "raises an error if a short_option was used twice" do
          conflicting_options = [
            PantographCore::ConfigItem.new(key: :foo,
                                         short_option: "-f",
                                         description: "foo"),
            PantographCore::ConfigItem.new(key: :bar,
                                         short_option: "-f",
                                         description: "bar")
          ]

          expect do
            PantographCore::Configuration.create(conflicting_options, {})
        ...
        it "raises an error for unresolved conflict between options" do
          conflicting_options = [
            ...
            PantographCore::ConfigItem.new(key: :bar),
            PantographCore::ConfigItem.new(key: :oof)
          ]
          ...
          expect do
            PantographCore::Configuration.create(conflicting_options, values)
      ...
      describe "#sensitive flag" do
        before(:each) do
          allow(PantographCore::Helper).to receive(:test?).and_return(false)
          allow(PantographCore::UI).to receive(:interactive?).and_return(true)
          allow(PantographCore::Helper).to receive(:ci?).and_return(false)
        end
        ...
        it "should ask using asterisks" do
          ...
          expect(PantographCore::UI).to receive(:password).and_return("password")
        ...
        it "should ask using plaintext" do
          config_item = PantographCore::ConfigItem.new(key: :foo,
                                                     ...
                                                     sensitive: false)
          config = PantographCore::Configuration.create([config_item], {})
          expect(PantographCore::UI).to receive(:input).and_return("plaintext")
      ...
      describe "arrays" do
        ...
        it "returns Array input values correctly" do
          config_item = PantographCore::ConfigItem.new(key: :foo,
                                                     description: 'foo',
                                                     type: Array)
          config = PantographCore::Configuration.create([config_item], { foo: ['5', '4', '3', '2', '1'] })
      ...
      describe "auto_convert_value" do
        ...
        it "auto converts Array values to Strings if allowed" do
          config_item = PantographCore::ConfigItem.new(key: :xcargs,
                                                     description: 'xcargs',
                                                     type: :shell_string)
        ...
        it "auto converts Hash values to Strings if allowed" do
          ...
          expected = "\"FOO BAR\"=\"I'm foo bar\" BAZ=\"And I'm baz\"" if PantographCore::Helper.windows?
        ...
        it "auto converts booleans as strings to booleans" do
          c = [
            PantographCore::ConfigItem.new(key: :true_value, is_string: false),
            PantographCore::ConfigItem.new(key: :true_value2, is_string: false),
            PantographCore::ConfigItem.new(key: :false_value, is_string: false),
            PantographCore::ConfigItem.new(key: :false_value2, is_string: false)
          ]
        ...
        it "doesn't auto convert booleans as strings to booleans if type is explicitly set to String" do
          c = [
            PantographCore::ConfigItem.new(key: :true_value, type: String),
            PantographCore::ConfigItem.new(key: :true_value2, type: String),
            PantographCore::ConfigItem.new(key: :false_value, type: String),
            PantographCore::ConfigItem.new(key: :false_value2, type: String)
          ]
        ...
        it "auto converts strings to integers" do
          c = [
            PantographCore::ConfigItem.new(key: :int_value,
                                         type: Integer)
          ]
          config = PantographCore::Configuration.create(c, {
            int_value: "10"
      ...
      describe "validation" do
        it "raises an exception if the data type is not as expected" do
          ...
          end.to raise_error(PantographCore::Interface::PantographError, "'foo' value must be a Float! Found String instead.")
        end

        it "verifies the default value as well" do
          c = PantographCore::ConfigItem.new(key: :output,
        ...
        it "calls verify_block for non nil values" do
          c = PantographCore::ConfigItem.new(key: :param,
                                      env_name: "PARAM",
        ...
        it "fails verification for a improper value set as an environment variable" do
          c = PantographCore::ConfigItem.new(key: :test_key,
                                    env_name: "TEST_KEY",
      ...
      describe "deprecation", focus: true do
        ...
        it "raises an exception if a deprecated option is not optional" do
          expect do
            config_item = PantographCore::ConfigItem.new(key: :foo,
                                                       ...
                                                       optional: false,
                                                       deprecated: 'replaced by bar')
          end.to raise_error(PantographCore::Interface::PantographCrash, 'Deprecated option must be optional')
        end

        it "doesn't display a deprecation message when loading a config if a deprecated option doesn't have a value" do
          c = PantographCore::ConfigItem.new(key: :foo,
                                           description: 'foo',
                                           deprecated: 'replaced by bar')
          values = {
            foo: "something"
          }
          expect(PantographCore::UI).to receive(:deprecated).with("Using deprecated option: '--foo' (replaced by bar)")
          config = PantographCore::Configuration.create([c], values)
        end
        ...
        it "displays a deprecation message when loading a config if a deprecated option has a value" do
          ...
          expect(PantographCore::UI).not_to(receive(:deprecated))
          config = PantographCore::Configuration.create([c], {})
        end
      end
      ...
      describe "misc features" do
        ...
        it "can push and pop configuration values" do
          name = PantographCore::ConfigItem.new(key: :name)
          platform = PantographCore::ConfigItem.new(key: :platform)
          other = PantographCore::ConfigItem.new(key: :other)

          config = PantographCore::Configuration.create([name, other, platform], {})
      ...
      describe "Automatically removes the --verbose flag" do
        it "removes --verbose if not an available options (e.g. a tool)" do
          config = PantographCore::Configuration.create([], { verbose: true })
        ...
        it "doesn't remove --verbose if it's a valid option" do
          ...
          config = PantographCore::Configuration.create(options, { verbose: true })
      ...
      describe "Use a valid Configuration Manager" do
        before do
          @options = [
            ...
            PantographCore::ConfigItem.new(key: :wait_processing_interval,
                                short_option: "-k",
        ...
        describe "#values" do
          ...
          it "returns the default values" do
            @config = PantographCore::Configuration.create(@options, {}) # no user inputs
            values = @config.values
        ...
        describe "fetch" do
          ...
          #   config_item = PantographCore::ConfigItem.new(key: :app_identifiers,
          #                            short_option: "-a",
          ...
          #                            default_value: "app.identifier",
          #                            default_value_dynamic: true)
          #   config = PantographCore::Configuration.create([config_item], {})

          #   config.set(:app_identifiers, nil)
          #   expect(PantographCore::UI).to receive(:input).and_return("app.identifier")
        ...
        describe "Parameter priority order" do
          it "prioritizes CLI values over everything else" do
            ENV["abc"] = "val env"
            config_item = PantographCore::ConfigItem.new(key: :item, env_name: "abc", default_value: "val default")
            config = PantographCore::Configuration.create([config_item], { item: "val cli" })
          ...
          it "asks if no other option" do
            ...
            config_item = PantographCore::ConfigItem.new(key: :item, env_name: "abc", optional: false)
            config = PantographCore::Configuration.create([config_item], {})

            expect(PantographCore::UI).to receive(:input).and_return("val ask")
