...
def before_each_pantograph_core
  ...
  ["invalid", "", 0, '284882215', ['338986109', 'FR']].each do |current|
    if current.kind_of?(Array)
      ...
      body_file = "pantograph_core/spec/responses/itunesLookup-#{id}_#{country}.json"
    else
      id = current
      url = "https://itunes.apple.com/lookup?id=#{id}"
      body_file = "pantograph_core/spec/responses/itunesLookup-#{id}.json"
    end
  ...
  stub_request(:get, "https://itunes.apple.com/lookup?bundleId=com.facebook.Facebook").
    with(headers: { 'Accept' => '*/*', 'Accept-Encoding' => 'gzip;q=1.0,deflate;q=0.6,identity;q=0.3', 'User-Agent' => 'Ruby' }).
    to_return(status: 200, body: File.read("pantograph_core/spec/responses/itunesLookup-com.facebook.Facebook.json"), headers: {})

  stub_request(:get, "https://itunes.apple.com/lookup?bundleId=net.sunapps.invalid").
    with(headers: { 'Accept' => '*/*', 'Accept-Encoding' => 'gzip;q=1.0,deflate;q=0.6,identity;q=0.3', 'User-Agent' => 'Ruby' }).
    to_return(status: 200, body: File.read("pantograph_core/spec/responses/itunesLookup-net.sunapps.invalid.json"), headers: {})
end
