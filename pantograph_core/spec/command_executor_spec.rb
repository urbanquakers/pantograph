describe PantographCore do
  describe PantographCore::CommandExecutor do
    describe "execute" do
      it 'executes a simple command successfully' do
        unless PantographCore::Helper.windows?
          expect(Process).to receive(:wait)
        end

        result = PantographCore::CommandExecutor.execute(command: 'echo foo')
      ...
      it 'handles reading which throws a EIO exception', requires_pty: true do
        ...
        result = PantographCore::CommandExecutor.execute(command: 'ls')
  ...
  - Muffins
    ...
    describe "which" do
      ...
      it "does not find commands which are not on the PATH" do
        expect(PantographCore::CommandExecutor.which('not_a_real_command')).to be_nil
      end
      ...
      it "finds commands without extensions which are on the PATH" do
        Tempfile.open('foobarbaz') do |f|
          ...
          PantographSpec::Env.with_env_values('PATH' => temp_dir) do
            expect(PantographCore::CommandExecutor.which(temp_cmd)).to eq(f.path)
          end
        end
      end
      ...
      it "finds commands with known extensions which are on the PATH" do
        Tempfile.open(['foobarbaz', '.exe']) do |f|
          ...
          PantographSpec::Env.with_env_values('PATH' => temp_dir, 'PATHEXT' => '.exe') do
            expect(PantographCore::CommandExecutor.which(temp_cmd)).to eq(f.path)
          end
        end
      end
      ...
      it "does not find commands with unknown extensions which are on the PATH" do
        Tempfile.open(['foobarbaz', '.exe']) do |f|
          ...
          PantographSpec::Env.with_env_values('PATH' => temp_dir, 'PATHEXT' => '') do
            expect(PantographCore::CommandExecutor.which(temp_cmd)).to be_nil
          end
        end
      end
    end
