...
For detailed instructions on how to get started with contributing to _pantograph_, first check out [YourFirstPR.md][first-pr] and [Testing.md](Testing.md). This guide will focus on more advanced instructions on how to debug _pantograph_ and _spaceship_ issues and work on patches.
...
As debugging with pry requires the development dependencies, make sure to execute _pantograph_ using `bundle exec` after running `bundle install` in the project- or _pantograph_ directory.
...
We’re not using the built-in network tracker of your browser, since we also need a proxy for our local _pantograph_ install, which will be covered in the next section of this document.
...
```
SPACESHIP_DEBUG=1 bundle exec pantograph your_normal_command
```
...
task :debug do
  ...
  # first login
  Spaceship::Tunes.login("apple@pantograph.tools") # use your own test account
  # or
  Spaceship::Portal.login("apple@pantograph.tools") # use your own test account
