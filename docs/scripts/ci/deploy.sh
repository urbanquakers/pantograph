...
echo "Starting deploy to https://docs.pantograph.tools"
...
# Bots need names too
git config --global user.email "pantographbothelper@krausefx.com"
git config --global user.name "pantograph bot"
...
git reset --hard HEAD # we don't want the `git checkout` to cause issues (e.g. https://circleci.com/gh/pantograph/docs/730)
...
echo "docs.pantograph.tools" > "CNAME"
...
echo "Deployed successfully, check out https://docs.pantograph.tools"
