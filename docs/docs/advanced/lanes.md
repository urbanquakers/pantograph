...
lane :first_lane do |options|
  puts "If you run: `pantograph first_lane`"
...
`for_platform` gives you similar control based on the platform for which you have invoked _pantograph_. So, for an `Appfile` configured like:
...
you can expect the `app_identifier` to equal `"com.forplatform.mac.forlane.release"` when invoking `pantograph mac release`.
