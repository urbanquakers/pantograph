...
GEMS = %w(pantograph danger-device_grid)

SECONDS_PER_DAY = 60 * 60 * 24

task(:rubygems_admins) do
  names = ["KrauseFx", "joshdholtz", "snatchev", "powerivq"]
  (GEMS + ["krausefx-shenzhen", "commander-pantograph", "pantograph-plugin-firebase_test_lab"]).each do |gem_name|
    names.each do |name|
...
task(:test_all) do
  ...
  formatter += " -r rspec_junit_formatter --format RspecJunitFormatter -o #{ENV['CIRCLE_TEST_REPORTS']}/rspec/pantograph-junit-results.xml" if ENV["CIRCLE_TEST_REPORTS"]
  sh("rspec --pattern ./**/*_spec.rb #{formatter}")
end

# Overwrite the default rake task
# since we use pantograph to deploy pantograph
task(:push) do
  sh("bundle exec pantograph release")
end
...
task(:update_gem_spec_authors) do
  ...
  gemspec = File.read("pantograph.gemspec")
  names = names.join("\",\n                        \"")
  gemspec.gsub!(/spec.authors\s+\=\s.*?\]/m, "spec.authors       = [\"#{names}\"]")
  File.write("pantograph.gemspec", gemspec)
end
